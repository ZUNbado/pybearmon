...
class SimpleMysql(object):
	...
	def connect(self):
		...
		try:
			self.conn = MySQLdb.connect(db=self.conf['db'], host=self.conf['host'], port=self.conf['port'],
	...
	def query_named(self, sql, params = None):
		"""Run a raw query making namedtuples"""
